...
이 프로젝트는 **ROS2 Humble** 환경에서 구동되며, 주요 의존성은 다음과 같습니다.

빠른 설치는 requirements.txt를 이용해주세요.

### 🧠 AI & Computer Vision
* **ultralytics**: YOLOv8/v11을 이용한 객체 탐지 (Fire, Fruit, Tools)
* **mediapipe**: 제스처 제어를 위한 손 추적(Hand Tracking)
* **opencv-python**: 실시간 영상 처리 및 이미지 조작
* **torch / torchvision**: 모델 추론 및 연산 가속

### 🌐 WebRTC & Communication
* **aiortc**: Python 기반 WebRTC 스택 (미디어 스트리밍)
* **aiohttp**: 시그널링 서버 통신을 위한 비동기 HTTP 라이브러리
* **Flask**: 웹 인터페이스 및 데이터 교환용 API 서버

### 🤖 Robot Control & Math
* **pymodbus==2.5.3**: Indy7 등 코봇 제어를 위한 Modbus 통신
* **numpy / scipy**: 고성능 수치 계산 및 좌표 변환
* **shapely / alphashape**: 2D 기하학 연산 및 영역 추출

### ⚙️ System Requirements
* **ROS2 Humble**: 메인 로봇 오케스트레이션 및 통신
* **Node.js & npm**: Vite 프론트엔드 빌드 및 실행
