        ...
        try:
            ...
            if status == "found":
                ...
                if voice_intent_sent:
                    # workspace_scan_node가 모든 픽 완료 후 발행
                    # → is_scanning은 finally에서 즉시 해제
                    print("  📡 scan_node 직접 처리 완료 (모든 픽 완료) → is_scanning 해제")
                else:
                    # fallback: publisher 큐를 통해 순서대로 픽 처리
            ...
            # scan_result 수신 시 항상 is_scanning 해제
            # voice_intent_sent=True: 모든 픽 완료 후 발행 → 즉시 해제
            # voice_intent_sent=False: 큐 워커가 처리 → is_robot_busy()로 추적
