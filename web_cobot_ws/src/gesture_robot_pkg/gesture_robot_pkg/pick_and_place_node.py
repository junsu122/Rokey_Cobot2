...
class PickAndPlaceNode(Node):
    ...
    def __init__(self):
        ...
        # APPROACH 완료 후 angle 파이프라인 재트리거용 (일반 픽에서만 사용)
        self._pub_selected_object    = self.create_publisher(
            SelectedObject, '/selected_object', 10)
        self._current_goal_label     = ''
        self._current_goal_from_scan = False
...
####################################################### 예외처리를 위한 코드        
    ...
    async def _execute_cb(self, goal_handle):
        ...
        self._current_goal_from_scan = bool(goal.from_scan)
    ...
    async def _execute_inner(self, goal_handle, goal, result):
        ...
        # ── 각도 리셋 전략 ────────────────────────────────────────────────
        # from_scan=True : workspace_scan_node가 센터링 후 /selected_label 발행
        #                  → 각도 파이프라인 pre-compute → 각도 보존 (리셋 안 함)
        # from_scan=False: APPROACH 후 /selected_object 재발행으로 각도 새로 계산
        #                  → 이전 픽 각도 차단을 위해 리셋
        if not from_scan:
            self._spin_angle        = 0.0
            self._angle_received_at = 0.0
        else:
            self.get_logger().info(
                f'[from_scan] 각도 보존: {self._spin_angle:.1f}° '
                f'(workspace_scan_node pre-computed)')
    ...
    async def _stage_approach(self, pt: list, h: float):
        ...
        # ── 일반 픽(from_scan=False)만 APPROACH 후 /selected_object 재발행 ──
        # from_scan=True : workspace_scan_node가 센터링 시점에 각도 pre-compute → 보존
        # from_scan=False: APPROACH 후 재발행으로 물체 위에서 정확한 각도 계산
        if not self._current_goal_from_scan:
            self._angle_received_at = 0.0
            self._spin_angle        = 0.0
            so_msg              = SelectedObject()
            so_msg.header.stamp = self.get_clock().now().to_msg()
            so_msg.label        = self._current_goal_label
            so_msg.confidence   = 1.0
            so_msg.box          = [int(v) for v in self._current_goal_box]
            self._pub_selected_object.publish(so_msg)
            self.get_logger().info(
                f'[APPROACH] /selected_object 재발행 → angle 파이프라인 트리거 '
                f'label={self._current_goal_label}')
