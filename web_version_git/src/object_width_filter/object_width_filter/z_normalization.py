...
>>>>>>> 18efdc04c5a8d8039eff383912863f9e5b7306ce
from visualization_msgs.msg import Marker, MarkerArray
from geometry_msgs.msg import Point
import sensor_msgs_py.point_cloud2 as pc2
import numpy as np
...
<<<<<<< HEAD
        ...
        self.alpha = 20.0
=======
        self.subscription = self.create_subscription(PointCloud2, '/no_ground', self.listener_callback, 10)
        self.publisher = self.create_publisher(PointCloud2, '/z_normalization', 10)
        self.count_publisher = self.create_publisher(Int32, '/object_count', 10)
        self.marker_publisher = self.create_publisher(MarkerArray, '/z_edge_marker', 10)
        self.xyz_publisher = self.create_publisher(PointCloud2, '/xyz_pointcloud', 10)
        
        # 🚀 Depth 데이터 발행 추가
        self.depth_info_pub = self.create_publisher(Float32MultiArray, '/object_depth_data', 10)

        self.get_logger().info("🚀 [Z-Normalization] 가동 및 Depth 데이터 발행 준비 완료")

        self.eps = 0.01             
        self.min_cluster_size = 5   
        self.alpha = 20.0
        self.table_mm = 1000.0 # 💡 실제 테이블 높이에 맞춰 수정 필요
>>>>>>> 18efdc04c5a8d8039eff383912863f9e5b7306ce
...
=======
        ...
        for i in range(len(points)):
            ...
            while queue:
                curr = queue.pop(0); cluster.append(curr)
                diff = points - points[curr]
                dist_sq_xy = np.sum(diff[:, :2]**2, axis=1) 
                neighbors = np.where((dist_sq_xy < xy_eps**2) & (~visited))[0]
                for neighbor in neighbors:
...
=======
            ...
            normalized_points = []
            representative_xyz_list = []
            marker_array = MarkerArray()
            
            clear_marker = Marker(); clear_marker.action = Marker.DELETEALL
            marker_array.markers.append(clear_marker)

            for cluster_idx, cluster in enumerate(clusters):
                if len(cluster) < self.min_cluster_size: continue
                cluster_pts = points[cluster]
                
                try:
                    # Z값 최솟값(가장 윗면) 추출
                    top_z = float(np.min(cluster_pts[:, 2]))
                    cluster_pts[:, 2] = top_z

                    # 🚀 Depth 계산 및 발행
                    obj_top_mm = top_z * 1000.0
                    cam_depth_h = self.table_mm - obj_top_mm
                    depth_m = top_z
                    table_m = self.table_mm * 0.001

                    depth_msg = Float32MultiArray()
                    depth_msg.data = [float(cam_depth_h), float(depth_m), float(table_m)]
                    self.depth_info_pub.publish(depth_msg)

                    for pt in cluster_pts: normalized_points.append(pt)

                    points_2d = cluster_pts[:, :2]
                    if np.std(points_2d[:, 0]) < 1e-4 or np.std(points_2d[:, 1]) < 1e-4: continue

                    alpha_shape = alphashape.alphashape(points_2d, self.alpha)
                    if alpha_shape.is_empty or not hasattr(alpha_shape, 'geom_type'): continue
                    ...
                    avg_x, avg_y = float(np.mean(x_coords)), float(np.mean(y_coords))
                    representative_xyz_list.append([avg_x, avg_y, top_z])

                    marker = Marker()
                    marker.header = msg.header; marker.ns = "object_edges"; marker.id = cluster_idx
                    marker.type = Marker.LINE_STRIP; marker.action = Marker.ADD
                    marker.pose.orientation.w = 1.0; marker.scale.x = 0.004  
                    marker.color.g = 1.0; marker.color.a = 1.0
                    for cx, cy in zip(x_coords, y_coords):
                        marker.points.append(Point(x=float(cx), y=float(cy), z=top_z))
                    marker_array.markers.append(marker)

                except Exception: continue

            if representative_xyz_list:
                self.xyz_publisher.publish(pc2.create_cloud_xyz32(msg.header, representative_xyz_list))
            if normalized_points:
                self.publisher.publish(pc2.create_cloud_xyz32(msg.header, np.array(normalized_points).tolist()))
            if len(marker_array.markers) > 1:
                self.marker_publisher.publish(marker_array)
...
def main(args=None):
    rclpy.init(args=args)
    node = ZNormalizationEdgeNode(); rclpy.spin(node)
