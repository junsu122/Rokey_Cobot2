...
=======
        ...
        self.selected_obj_sub = self.create_subscription(SelectedObject, '/selected_object', self.selected_object_callback, 10)
        self.point_pub = self.create_publisher(PointStamped, '_2d_to_3d_point', 10)
        self.get_logger().info("🚀 실시간 좌표 스트리밍 노드가 준비되었습니다.")

    def depth_callback(self, msg):
        self.current_depth_frame = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')

    def selected_object_callback(self, msg: SelectedObject):
        if self.current_depth_frame is None: return
        x1, y1, x2, y2 = msg.box
        u, v = (x1 + x2) // 2, (y1 + y2) // 2
        ud, vd = int(u * 848/1280), int(v * 480/720)
        
        try:
            depth_mm = self.current_depth_frame[vd, ud]
            ...
            p_msg.header.frame_id = msg.label
            p_msg.point.x, p_msg.point.y, p_msg.point.z = x, y, z
            self.point_pub.publish(p_msg)
            self.get_logger().info(f"📤 [Once] {msg.label} 좌표 전송 완료.")
        except Exception: pass

def main(args=None):
    rclpy.init(args=args); node = SelectedObjectTo3DNode()
    try: rclpy.spin(node)
